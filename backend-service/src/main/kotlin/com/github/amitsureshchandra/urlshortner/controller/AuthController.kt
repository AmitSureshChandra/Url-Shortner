...
class AuthController(val authService: AuthService) {
    ...
    fun registerHandler(@RequestBody @Valid regDto: RegDto): MutableMap<String, Any> {
        return authService.register(regDto)
    }

    @PostMapping("/login")
    fun login(@RequestBody loginCredentials: LoginCredentials): MutableMap<String, Any> {
